#!/usr/bin/env python3
"""Run MP-SENet's pinned end-to-end real-speech quality regression."""
...
def read_wav(path: pathlib.Path) -> tuple[list[float], int]:
    with wave.open(str(path), "rb") as reader:
        if reader.getnchannels() != 1 or reader.getsampwidth() != 2:
            raise SystemExit("MP-SENet fixture must be 16-bit mono PCM")
...
def main() -> None:
    ...
    if not model.is_file():
        raise SystemExit(f"MP-SENet model not found: {model}")
    with tempfile.TemporaryDirectory(prefix="denoize-mpsenet-") as directory:
        ...
        subprocess.run(
            [
                ...
                "--backend", "mpsenet", "--onnx-model", str(model),
                "--onnx-rate", "16000",
            ],
            check=True,
        )
        ...
        if improvement < MINIMUM_SI_SNR_IMPROVEMENT_DB:
            raise SystemExit(
                "MP-SENet speech quality regression: "
