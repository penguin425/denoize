#!/usr/bin/env python3
"""Run BSRNN's reproducible end-to-end speech quality regression.

The clean two-second speech fixture comes from the Apache-2.0 ESPnet source
tree at the same revision recorded by the published checkpoint. The script
adds deterministic white noise, invokes the Rust CLI, and requires a material
scale-invariant SNR improvement.
"""
...
def read_wav(path: pathlib.Path) -> tuple[list[float], int]:
    with wave.open(str(path), "rb") as reader:
        if reader.getnchannels() != 1 or reader.getsampwidth() != 2:
            raise SystemExit("BSRNN fixture must be 16-bit mono PCM")
...
def main() -> None:
    ...
    if not model.is_file():
        raise SystemExit(f"BSRNN model not found: {model}")

    with tempfile.TemporaryDirectory(prefix="denoize-bsrnn-") as directory:
        ...
        if improvement < MINIMUM_SI_SNR_IMPROVEMENT_DB:
            raise SystemExit(
                "BSRNN speech quality regression: "
