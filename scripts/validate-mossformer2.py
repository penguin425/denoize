#!/usr/bin/env python3
"""Run MossFormer2's pinned end-to-end real-speech quality regression."""
...
MINIMUM_SI_SNR_IMPROVEMENT_DB = 1.0
...
def read_wav(path: pathlib.Path) -> tuple[list[float], int]:
    with wave.open(str(path), "rb") as reader:
        if reader.getnchannels() != 1 or reader.getsampwidth() != 2:
            raise SystemExit("MossFormer2 fixture must be 16-bit mono PCM")
...
def main() -> None:
    ...
    if not model.is_file():
        raise SystemExit(f"MossFormer2 model not found: {model}")
    with tempfile.TemporaryDirectory(prefix="denoize-mossformer2-") as directory:
        root = pathlib.Path(directory)
        clean_path, noisy_path, enhanced_path = (
            root / "clean.wav", root / "noisy.wav", root / "enhanced.wav"
        )
        ...
        # The converted deployment graph uses the official four-second
        # segment. Repeat the pinned two-second utterance to exercise a full
        # window without zero-padding most of the model context.
        clean = clean * 2
        generator = random.Random(NOISE_SEED)
        noisy = [
            max(-1.0, min(32767.0 / 32768.0, sample + generator.gauss(0.0, NOISE_AMPLITUDE)))
            for sample in clean
        ]
        write_wav(noisy_path, noisy, sample_rate)
        subprocess.run(
            [
                str(denoize), str(noisy_path), str(enhanced_path),
                "--backend", "mossformer2", "--onnx-model", str(model),
                "--onnx-rate", "48000",
            ],
            check=True,
        )
        ...
        if improvement < MINIMUM_SI_SNR_IMPROVEMENT_DB:
            raise SystemExit(
                "MossFormer2 speech quality regression: "
